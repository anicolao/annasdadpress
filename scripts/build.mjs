...
function familyGrid() {
  return `<div class="family-grid">${families.map((f, i) => `<a class="family-card" style="--accent:${f.color}" href="${url("/books/" + f.id + "/")}"><div class="family-top">${icon(f.icon, "family-icon")}<span class="index">0${i + 1}</span></div><h3>${f.name}</h3><h4>${f.purpose}</h4><p>${f.description}</p><span class="text-link">Explore the series ${arrow}</span></a>`).join("")}</div>`;
}
function choose() {
