...
function card(b) {
  ...
  return `<article class="book-card"><a class="book-art ${b.family}" href="${url("/books/" + b.slug + "/")}">${cover(b)}</a><div class="book-info"><span class="eyebrow" style="color:${f?.color || "#066568"}">${f?.name || "The complete course"}</span><h3>${link("/books/" + b.slug + "/", b.title)}</h3><p>${b.description}</p><span class="status">Forthcoming</span></div></article>`;
}
...
async function catalog(path, selected) {
  ...
  const selectedBooks = books.filter((b) => !f || b.family === selected);
  await page(
    path,
    f
      ? f.name + " Sudoku books"
...
for (const b of books) {
  ...
  await page(
    ...
    `<div class="wrap"><div class="breadcrumbs">${link("/books/", "Our books")} <span>/</span> ${f ? link("/books/" + f.id + "/", f.name) : "The complete course"}</div><section class="book-detail"><div class="detail-cover"><div class="book-art ${b.family}">${cover(b, true)}</div><p class="small-note">${b.coverStatus}</p></div><div><p class="eyebrow">${b.collection}</p><span class="status">${b.status}</span><h1>${b.title}</h1>${b.subtitle ? `<p class="subtitle">${b.subtitle}</p>` : ""}<p>${b.description}</p><div class="best-for"><span class="eyebrow">A good fit for</span><p>${b.bestFor}</p></div><div class="purchase"><h2>Coming to your bookshelf.</h2><p>Publication date to be announced. Amazon links will appear here when the book is available.</p><div class="marketplaces">${[
      ...
      .map(([key, label]) =>
        b[key]
          ? link(b[key], label + arrow, "button")
          : `<span>${label} <small>Coming soon</small></span>`,
      )
      .join(
        "",
      )}</div><dl><div><dt>ISBN</dt><dd>${b.isbn || "To be announced"}</dd></div><div><dt>ASIN</dt><dd>${b.asin || "To be announced"}</dd></div></dl></div></div></section>${guide ? `<section class="section teaching"><p class="eyebrow">Understanding, step by step</p><h2>Learn it. See it. Make it yours.</h2><div class="teaching-steps">${["Learn", "See", "Walkthrough", "Practice", "Solve"].map((t, i) => `<div><span>0${i + 1}</span><h3>${t}</h3></div>`).join("")}</div><div class="two-col"><div><h3>A complete visual course</h3><p>Planned as a full-colour course of approximately 250 pages, with visual explanations, complete walkthroughs, and practice that increases in difficulty.</p><p>The book's QR codes open the exact puzzle in our ${link("https://sudoku.annasdadpress.com/", "online Sudoku solver")}. Enter digits and candidate notes, ask for a hint, or follow a walkthrough. Find the solver any time at ${link("/next/", "annasdadpress.com/next")}.</p></div><div><h3>From the first rule to advanced logic</h3><p>Full Houses; Naked and Hidden Singles; Naked and Hidden Pairs; Pointing Pairs; Y-Wing; X-Wing; Swordfish; Naked Triples; Simple Colors; XY-Chains; Unique Rectangles; and 3D Medusa.</p></div></div></section>` : ""}${b.features ? `<section class="section"><p class="eyebrow">The first Discovery book</p><h2>One puzzle a day.<br><em>One picture to discover.</em></h2><div class="progression">${b.features.map((step, i) => `<div><span class="eyebrow">0${i + 1}</span><h3>${esc(step.title)}</h3><p>${esc(step.description)}</p></div>`).join("")}</div><p>Made for pencil and paper, with an 8 x 10 inch page format and printed drawing instructions.</p>${link("/books/discovery/", "Explore the Discovery series " + arrow, "text-link")}</section>` : ""}${interiors(b)}<section class="section"><div class="section-heading"><h2>More ways to learn.</h2>${link("/sudoku-learners-library/", "Explore the library " + arrow, "text-link")}</div><div class="book-grid">${books
