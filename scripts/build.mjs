...
export async function buildSite({
  ...
  const selections = JSON.parse(
    await readFile("src/samples/selections.json", "utf8"),
  );
  ...
  for (const b of books.filter((b) => b.sampleManifest)) {
    const record = JSON.parse(await readFile(b.sampleManifest, "utf8"));
    assert.equal(record.schema, 1);
    const key = b.cover === "practice" ? "practice-xwing" : b.cover;
    assert.equal(
      record.selectionSha256,
      createHash("sha256").update(canonicalJson(selections[key])).digest("hex"),
      "Sample selection changed; run samples:sync",
    );
    for (const [name, hash] of Object.entries(record.assets)) {
      assert.equal(
        createHash("sha256")
          .update(await readFile(`public/assets/samples/${name}`))
          .digest("hex"),
        hash,
        "Sample integrity check failed; run samples:sync",
      );
    }
  }
  ...
  function interiors(b) {
    if (!b.sampleSpreads.length) return "";
    return `<section class="interior interior-preview section" id="sample-pages"><p class="eyebrow">A look inside</p>${b.sampleSpreads.map((spread) => `<h2>${esc(spread.title)}</h2><p class="spread-intro">${esc(spread.description)}</p><figure class="sample-spread"><div class="spread-pages">${spread.pages.map((p) => `<a class="sample-page" href="${url("/assets/samples/" + p.image + "-1280.webp")}" aria-label="${esc(p.label)}: view full-size image"><img src="${url("/assets/samples/" + p.image + "-640.webp")}" srcset="${url("/assets/samples/" + p.image + "-640.webp")} 640w, ${url("/assets/samples/" + p.image + "-1280.webp")} 1280w" sizes="(max-width: 650px) calc(100vw - 80px), (max-width: 1050px) 40vw, 520px" width="${p.width}" height="${p.height}" loading="lazy" alt="${esc(p.alt)}"><span>${esc(p.label)} ${arrow}</span></a>`).join("")}</div><figcaption>${esc(spread.caption)}</figcaption></figure><div class="spread-actions">${link(spread.pdf, "Read the two-page sample (PDF) " + arrow, "button")}<span>Choose a page to view it full size.</span></div><details class="sample-explanation"><summary>Read the example in text</summary>${spread.explanation.map((p) => `<p>${esc(p)}</p>`).join("")}</details>`).join("")}</section>`;
  }
  function familyGrid() {
    return `<div class="family-grid">${families.map((f, i) => `<a class="family-card ${f.id}" style="--accent:${f.color}" href="${url("/books/" + f.id + "/")}"><div class="family-copy"><div class="family-top">${icon(f.icon, "family-icon")}<span class="index">0${i + 1}</span></div><h3>${f.name}</h3><h4>${f.purpose}</h4><p>${f.description}</p><span class="text-link">Explore the series ${arrow}</span></div>${f.id === "discovery" ? `<div class="family-cover">${cover(books.find((b) => b.family === "discovery"))}</div>` : ""}</a>`).join("")}</div>`;
  }
  ...
  async function page(path, title, description, body, schema) {
    ...
    const html = `<!doctype html><html lang="en-CA"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)} | Anna's Dad Press</title><meta name="description" content="${esc(description)}"><link rel="canonical" href="${canonical}"><meta property="og:type" content="${schema ? "book" : "website"}"><meta property="og:site_name" content="Anna's Dad Press"><meta property="og:title" content="${esc(title)}"><meta property="og:description" content="${esc(description)}"><meta property="og:url" content="${canonical}"><meta property="og:image" content="${schema ? origin + "/assets/social/" + books.find((b) => b.title === schema.name).cover + ".png" : origin + "/assets/social.png"}"><meta property="og:image:alt" content="${esc(schema?.name || "Anna's Dad Press: Thoughtfully made books")}"><meta name="twitter:card" content="summary_large_image"><meta name="theme-color" content="#f7f6f0"><link rel="icon" href="${url("/assets/favicon.svg")}" type="image/svg+xml"><link rel="stylesheet" href="${url("/assets/style.css")}"><script type="application/ld+json">${JSON.stringify(schema || organization).replaceAll("<", "\\u003c")}</script></head><body><a class="skip" href="#main">Skip to content</a><header class="site-header"><div class="wrap header-inner">${link("/", `${mark}<span class="brand-name">Anna's Dad<span>PRESS</span></span>`, "brand")}<nav aria-label="Main navigation">${[
      ...
      .join(
        "",
      )}${link("/next/", "Reader resources " + arrow, "nav-cta")}</nav></div></header><main id="main" tabindex="-1">${body}</main><footer><div class="wrap footer-main"><div>${link("/", `${mark}<span class="brand-name">Anna's Dad<span>PRESS</span></span>`, "brand")}<p>Thoughtfully made books.</p></div><div class="footer-links">${link("/books/", "Our books")}${link("/about/", "Our story")}${link("/next/", "Reader resources")}${link("/privacy/", "Privacy")}${contact}</div></div>${hasAffiliateLinks ? `<p class="wrap affiliate-disclosure">${affiliateDisclosure}</p>` : ""}<div class="wrap footer-bottom"><span>Copyright ${new Date().getFullYear()} Anna's Dad Press</span><span>Independent publishing / Canada</span></div></footer></body></html>`;
    const file =
      path === "/404.html"
        ? `${outputDir}/404.html`
  ...
  await page(
    ...
    `${intro("A collection built around learning", "The Sudoku<br><em>Learner's Library.</em>", "A good puzzle asks you to think. A good learning book helps you understand how. Find the right balance of explanation, practice, and independence.")}<section class="wrap section top-zero"><div class="section-heading"><div><p class="eyebrow">Different books. Different jobs.</p><h2>A library, not a ladder.</h2></div><p>Use the Guide as your reference. Choose Practice for a specific  technique, Start Here for a hint, or Candidates Done to skip  setup. Mastery gives you room to solve on your own. Discovery adds a creative reward: puzzles that reveal a picture.</p></div>${familyGrid()}${choose()}</section>`,
  );
